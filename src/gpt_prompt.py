...
def get_risk_score(news, model="gpt-3.5-turbo", company=None, output="score", explanation=False):
    ...
    if company:
        ...
        company_prompt = ""
    if explanation:
        explanation_prompt = f"Your answer will include 2 lines. In the first line, you will answer 1 sentence to analyze why the news might be risky {company_prompt}."
        explanation_addup = "in the second line, "
    else:
        explanation_prompt = "You will not give explanation to your answer."
        explanation_addup = ""
    ...
    if output == "polar":
        output_prompt = f"Then, {explanation_addup}you will answer with: RISKY, NEUTRAL, SAFE."
    elif output =="score":
