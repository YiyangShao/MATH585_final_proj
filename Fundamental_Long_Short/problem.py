...
class BasicTemplateAlgorithm(QCAlgorithm):
    ...
    def OnData(self, data):
        ...
        for security in self.Portfolio.Values:
            if security.Invested:
                # Calculate the stop-loss price for long and short positions
                symbol = security.Symbol
                if security.IsLong:
                    stop_loss_price = security.AveragePrice * (1 - self.stop_loss_percent)
                    if symbol in data and data[symbol] is not None and data[security.Symbol].Price < stop_loss_price:
                        self.Liquidate(security.Symbol)
                        self.Debug(f"Long stop-loss triggered for {security.Symbol} at {data[security.Symbol].Price}")
                
                elif security.IsShort:
                    stop_loss_price = security.AveragePrice * (1 + self.stop_loss_percent)
                    if symbol in data and data[symbol] is not None and data[security.Symbol].Price > stop_loss_price:
                        self.Liquidate(security.Symbol)
                        self.Debug(f"Short stop-loss triggered for {security.Symbol} at {data[security.Symbol].Price}")
    ...
    # Assign each stock equally. Alternatively you can design your own portfolio construction method
        for i in self.long:
            self.SetHoldings(i, self.holding_coef/self.num_fine)
